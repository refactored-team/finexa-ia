...
from pipeline.bedrock import invoke_classification, MODEL_SONNET
...
from pipeline.heuristics import classify_heuristic, fallback_classify
from pipeline.logger import get_logger
from pipeline.prompts import CLASSIFICATION_SYSTEM_PROMPT
...
BATCH_SIZE = 20  # max transacciones por llamada a Bedrock (presupuesto de contexto)
...
async def _classify_single_batch(
    ...
    Puede ejecutarse en paralelo con otros batches vía asyncio.gather().
    """
    try:
        ...
        for i, tx in enumerate(batch):
            ...
            if ai_result:
                ...
                # Calentar caché para este merchant
        ...
        # Bedrock totalmente no disponible — fallback para todo el batch
        logger.error(
...
async def classify_batch(
    ...
    if needs_bedrock:
        ...
        # Todos los batches se lanza en paralelo — el más lento determina el tiempo total
    ...
    from pipeline.validators import post_validate_batch
