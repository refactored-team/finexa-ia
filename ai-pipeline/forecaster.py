...
# Categories considered discretionary / impulse-prone
_DISCRETIONARY = {
...
# Skipped for recurring detection (income, transfers, pending)
...
def _best_datetime(tx: EnrichedTransaction) -> Optional[datetime]:
    ...
    Uses tx.datetime (which was populated from authorized_datetime or datetime in Plaid).
    Falls back to midnight on tx.date.
    """
    if tx.datetime:
        try:
...
def _has_intraday_time(tx: EnrichedTransaction) -> bool:
    """True if the transaction carries sub-day time information."""
    return bool(tx.datetime and "T" in tx.datetime)


def _detect_periodicity(gaps: list[int]) -> tuple[int, str]:
    """
    Given day-gaps between consecutive transactions, return (days, label).
...
def detect_recurring_expenses(
    ...
    """
    Detecta gastos recurrentes agrupando por comercio y verificando:
      - Mismo comercio (merchant normalizado)
      - Monto dentro de ±amount_tolerance del promedio (default 15%)
      - Al menos min_occurrences apariciones en el periodo

    Args:
        transactions:     Transacciones clasificadas.
        today:            Fecha de referencia (default: hoy).
        horizon_days:     Ventana futura para marcar is_pending.
        min_occurrences:  Mínimo de ocurrencias para considerar recurrente.
        amount_tolerance: Tolerancia de variación del monto (fracción, default 0.15).

    Returns:
        Lista de RecurringExpense ordenada por avg_amount descendente.
    """
    today = today or date.today()

    # Only outgoing, non-skip transactions
    expenses = [
    ...
    for key, group in by_merchant.items():
        ...
        # Canonical display name: prefer non-null merchant_name
        display = (
...
def compute_liquidity_forecast(
    ...
    """
    Calcula la liquidez proyectada y genera alerta si cae bajo el umbral.

    Fórmula: liquidez_estimada = saldo_actual − Σ(avg_amount de recurrentes pendientes)

    Args:
        recurring_expenses:  Gastos recurrentes detectados.
        saldo_actual:        Saldo actual de la cuenta.
        threshold:           Umbral de alerta. Default: 10% de ingresos o $500.
        ingresos_mensuales:  Ingreso mensual declarado (para umbral automático).
        horizon_days:        Horizonte de proyección en días.

    Returns:
        (liquidez_estimada, LiquidityAlert | None)
    """
    ...
    # Alert: liquidity below threshold
...
def detect_impulse_spending(
    ...
    """
    Detecta ráfagas de gasto impulsivo: min_count+ gastos discrecionales en
    una ventana de window_hours horas.

    Cuando las transacciones no tienen dato de hora (solo fecha), la ventana
    se degrada a 24 horas (mismo día) para no perder detecciones válidas.

    Args:
        transactions:  Transacciones clasificadas (con datetime si disponible).
        window_hours:  Tamaño de la ventana temporal en horas (default 4).
        min_count:     Mínimo de gastos para disparar alerta (default 3).

    Returns:
        Lista de ImpulseSpendingAlert sin ventanas duplicadas.
    """
    discretionary = [
        tx for tx in transactions
    ...
    # Build (tx, datetime) pairs; skip txs without a parseable date
    ...
    # Effective window: use actual hours only if any tx has intraday time data
    effective_window = timedelta(hours=window_hours if any_intraday else 24)

    alerts: list[ImpulseSpendingAlert] = []
    # Track end of last reported window to avoid overlapping alerts
    ...
    while i < len(timed):
        ...
        # Skip if this transaction is already covered by a previous alert window
...
def run_cash_flow_analysis(
    ...
    Args:
        ...
        saldo_actual:         Saldo actual de la cuenta.
        threshold:            Umbral de alerta de liquidez (default: automático).
        ingresos_mensuales:   Ingresos declarados (para threshold automático).
        horizon_days:         Horizonte de proyección en días (default 30).
        impulse_window_hours: Ventana para detección de impulsos (default 4h).
        today:                Fecha de referencia (default: hoy).

    Returns:
        CashFlowResult con recurrentes, proyección y alertas.
    """
